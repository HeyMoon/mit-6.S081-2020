...
usertrap(void)
{
  ...
  }else if (r_scause() == 13 || r_scause() == 15)
  {
    ...
    else
    {
      ...
      {
        if(p->vmas[i].used && p->vmas[i].address <= va && va < (p->vmas[i].address + p->vmas[i].length))
        {
          ...
          int offset = PGROUNDDOWN(va - p->vmas[i].address);
          int cnt = readi(p->vmas[i].file->ip, 0, pa, offset, PGSIZE);
